...
const exampleNote = await model.loadTile("exampleProfile");
const newNote = await model.createTile("Profile", {
  firstName: "Andrew",
  lastName: "Sesti",
  favoriteNumber: 1,
});
await store.set("ProfileDefinition", {
  firstName: "First",
  lastName: "Last",
  favoriteNumber: 0,
});

const tile = await store.get("ProfileDefinition");
